...
public static class Common
{
    ...
    public static string ToJson(DataSet dataSet)
    {
        ...
        foreach (DataTable table in dataSet.Tables)
        {
            if (table.Rows.Count > 0)
                jsonString += "\"" + table.TableName + "\":" + ToJsonEx(table) + ",";
        }
    ...
    public static string ToJsonEx(DataTable dt)
    {
        StringBuilder jsonString = new StringBuilder();
        jsonString.Append("[");
    ...
    public static void SetPageNoCache()
    {
        ...
        HttpContext.Current.Response.AddHeader("Pragma", "");
    }
    #endregion
    ...
    #region 将object数组转化为string
    /// <summary>
    /// 将object数组转化为string
    /// </summary>
    /// <param name="oArray">object数组</param>
    /// <returns></returns>
    public static string[] ObjarrToStrarr(object[] oArray)
    {
        return Array.ConvertAll<object, string>(oArray, Convert.ToString);
    }
    #endregion
    ...
    /// 通用调用存储过程方法
    /// </summary>
    /// <param name="MethodsName">调用方法名</param>
    /// <param name="sArray">数据库调用参数，string数组</param>
    /// <returns>数据库方法调用返回值</returns>
    public static string[] CommonUsingMethods(string MethodsName, string[] sArray)
    {
        object Result = null;
        object[] obj = null;
        try
        {
            Type tp = Type.GetType("DotNet.Utilities.ComSqlProc");//命名空间.类名
            object ctemp = Activator.CreateInstance(tp);//通过反射创建类
            MethodInfo mi = tp.GetMethod(MethodsName);//方法的名称           
            Result = mi.Invoke(ctemp, new object[] { sArray });
            obj = (object[])Result;
        }
        catch (Exception e)
        {
    ...
    /// 通过接口发送短信验证
    /// </summary>
    /// <param name="Text">短信{1}中取代的位置</param>
    /// <param name="tele">接收短信的手机号码</param>
    public static bool postMessage(string Text, string tele, ref string ret)
    {
        bool bol = false;
        CCPRestSDK.CCPRestSDK api = new CCPRestSDK.CCPRestSDK();
        bool isInit = api.init("app.cloopen.com", "8883");
        api.setAccount("aaf98f8951f7362501520b59098a1e8f", "5f1677004d6b41c5b3ea02c82114ecd6");
        api.setAppId("8a48b55151f715fb01520b5c3c621fab");
        try
        {
            if (isInit)
            {
                Dictionary<string, object> retData = api.SendTemplateSMS(tele, "61840", new string[] { Text, "3" });
                //statusCode=000000;statusMsg=成功;data={TemplateSMS={dateCreated=20150811082550;smsMessageSid=201508110825501638513;};};
                ret = getDictionaryData(retData).Split(';')[0].Split('=')[1];
                if (ret == "000000")
                    bol = true;
                else
                    ret = getDictionaryData(retData).Split(';')[1].Split('=')[1];
            }
            else
            {
                ret = "初始化失败";
    ...
    public static void Write(string title, string[] sarray,string sp)
    {
        string Message = string.Join(sp, sarray);
        StreamWriter sw = null;
        DateTime date = DateTime.Now;
        string FileName = DateTime.Now.ToString("yyyy-MM-dd");
        try
